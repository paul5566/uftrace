...
void prepare_pmu_event(enum uftrace_event_id id)
{
	struct pmu_data *pd;
	...
	case EVENT_ID_PMU_CYCLE:
		...
		if (pd->fd[0] < 0)
			pr_err("failed to open 'cpu-cycles' perf event");

		pd->fd[1] = open_perf_event(PERF_TYPE_HARDWARE,
					    PERF_COUNT_HW_INSTRUCTIONS);
		if (pd->fd[1] < 0)
			pr_err("failed to open 'instructions' perf event");
		break;
...
void read_pmu_event(enum uftrace_event_id id, void *buf)
{
	struct pmu_data *pd;
	...
	if (list_no_entry(pd, &pmu_fds, list))
		pr_err_ns("cannot find PMU setting for %d\n", id);
